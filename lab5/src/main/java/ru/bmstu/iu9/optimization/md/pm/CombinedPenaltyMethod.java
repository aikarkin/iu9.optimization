...
public class CombinedPenaltyMethod extends BasePenaltyMethod implements OptimizationMethod {

    public CombinedPenaltyMethod(Function<RealVector, Double> objectiveFunc, List<Function<RealVector, Double>> constraints, PatternSearchConfig psc, DichotomyMethodConfig dmc, PenaltyMethodConfig c) {
        super(objectiveFunc, constraints, psc, dmc, c);
    }

    @Override
    public OptimizationResult optimize(RealVector x0) {
        return super.optimize(COMBINED_PENALTY_FUNC, x0, c.betaInternal());
    }
